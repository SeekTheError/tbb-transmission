...
function downloadBestSeason(torrentName, fromSeason, fromEpisode, callback) {
  var episodeId = new EpisodeIdIterator(torrentName, fromSeason, fromEpisode);

  internals.$downloadBestSeason(episodeId, true, callback);
}

internals.$downloadBestSeason = function(episodeId, firstCall, callback) {
  if (!callback && _.isFunction(callback)) {
    callback = firstCall;
    firstCall = null;
  }

  downloadBestTorrent(episodeId.toString(), function(err, res) {
