...
var internals = {
  ...
  config: {
    transmission: null,
    torrentSearchUrl: 'http://thepiratebay.se/search/--search--/0/7/0',
  },
  /**
...
function createSearchUrl(query) {
  return internals.config.torrentSearchUrl.replace('--search--', encodeURIComponent(query));
}
...
function findBestTorrent(torrentName, callback) {
  // tbb most seeded url
  var url = createSearchUrl(torrentName);
  var res = request.get({
    url: url,
    headers: {
  ...
  gunzip.on('error', function(err) {
    callback(new Error('Failed to retrieve informations from website, url:' + url, err));
  });
}
...
function downloadTorrent(torrent, callback) {
  if (!internals.transmission) {
    internals.transmission = new Transmission(internals.config.transmission || null);
  }
  ...
  internals.transmission.addUrl(torrent.magnet, function(err, res) {
    if (err) {
...
function setConfig(config) {
  internals.config.torrentSearchUrl = config.torrentSearchUrl || internals.config.torrentSearchUrl;
  delete config.torrentSearchUrl;

  internals.transmission = null;
  internals.config.transmission = config;
}
